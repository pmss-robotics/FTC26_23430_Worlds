...
@Config
@com.qualcomm.robotcore.eventloop.opmode.TeleOp(name = "|B|Duo|SOTM|", group = "TeleOp")
public class BlueDuoSOTM extends CommandOpMode {
    ...
    public  static double       IND_PINK    = 0.722;
    ...
    public void initialize() {
        ...
        // D-Pad Left/Right: turret offset trim ±2.5°
        new GamepadButton(driver, GamepadKeys.Button.DPAD_LEFT).whenPressed(
                new InstantCommand(() -> offset += 2.5)
        );
        ...
        // X: relocalize — pose reset + pink flash
        ...
        schedule(new RunCommand(() -> {
            ...
            // Flywheel
            if (aimornot) {
                outtake.setVelocityRpm(Math.max(1965, computeY(distance) + rpmOffset));
            } else {
            ...
            if (isFlashing) {
                ...
                if (halfCycle >= FLASH_TOTAL) {
                    ...
                    indicator.setPosition((halfCycle % 2 == 0) ? IND_PINK : IND_DEFAULT);
                }
            } else if (error < 1.0) {
